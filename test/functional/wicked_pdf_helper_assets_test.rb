...
class WickedPdfHelperAssetsTest < ActionView::TestCase
  ...
  if Rails::VERSION::MAJOR == 4
    ...
    test 'wicked_pdf_asset_path should return an url with the same protocol when assets are served by an asset server with protocol defined' do
      expects(:asset_path => 'http://assets.domain.com/dummy.png')
      expects('precompiled_asset?' => true)
      assert_equal 'http://assets.domain.com/dummy.png', wicked_pdf_asset_path('dummy.png')
    end

    test 'wicked_pdf_asset_path should return an url with the same protocol when assets are served by an asset server with ssl protocol defined' do
